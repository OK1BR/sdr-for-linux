...
| Apache Labs **ANAN G2** (Saturn) | HPSDR Protocol 2 | 🟡 **RX only, not yet confirmed on air** — the P2 link we already ship plus the per-device wire bytes taken from piHPSDR (two Alex boards, two ADCs, DDC2, ANAN-7000 band-pass knees) and pinned by an offline gate. We do not own a G2, so RX is unlocked for testing while **TX and PureSignal stay locked** (their calibration must be walked in on the physical radio with a dummy load). Reports welcome — see `docs/RADIOS-SCOPE.md` |
...
- **Radios: ANAN G2E, ANAN 10E/100B, Hermes Lite 2 (RX + TX), ANAN G2/Saturn
  (RX only, awaiting on-air confirmation)** — whitelist, see above; one RX
  (RX2 planned)
