...
int settings_load(Settings *s) {
  ...
  if (g_key_file_load_from_file(kf, settings_path(), G_KEY_FILE_NONE, NULL)) {
    ...
    if (g_key_file_has_key(kf, GROUP_DISPLAY, "trace_cols", NULL))
      s->trace_cols = g_key_file_get_integer(kf, GROUP_DISPLAY, "trace_cols", NULL);
    ...
    if (g_key_file_has_key(kf, GROUP_DISPLAY, "spot_ttl", NULL))
      s->spot_ttl = g_key_file_get_integer(kf, GROUP_DISPLAY, "spot_ttl", NULL);
    ...
    if (g_key_file_has_key(kf, GROUP_TX, "ps_enable", NULL))
      s->ps_enable = g_key_file_get_integer(kf, GROUP_TX, "ps_enable", NULL);
    if (g_key_file_has_key(kf, GROUP_TX, "ps_att", NULL))
      s->ps_att = g_key_file_get_integer(kf, GROUP_TX, "ps_att", NULL);
    if (g_key_file_has_key(kf, GROUP_TX, "ps_setpk", NULL))
    ...
    if (g_key_file_has_key(kf, GROUP_TX, "ps_stbl", NULL))
      s->ps_stbl = g_key_file_get_integer(kf, GROUP_TX, "ps_stbl", NULL);
...
int settings_save(const Settings *s) {
  ...
  g_key_file_set_integer(kf, GROUP_DISPLAY, "trace_cols", s->trace_cols);
  ...
  g_key_file_set_integer(kf, GROUP_DISPLAY, "spot_ttl",   s->spot_ttl);
